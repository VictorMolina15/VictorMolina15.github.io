...
float fbm(vec2 p) {
  ...
  for (int i = 0; i < OCTAVES; i++) {
    // [NUEVO] EFECTO LÍQUIDO: Agregamos una "ondulación" base.
    // Esto hace que el fluido se deforme constantemente (hierva) 
    // sin importar qué tan lento sea el waveSpeed general.
    vec2 undulation = vec2(time * 0.15, time * 0.1);
    value += amp * abs(cnoise(p + undulation));
    p *= freq;
    amp *= waveAmplitude;
  }
  return value;
...
float pattern(vec2 p) {
  // [NUEVO] LA CORRIENTE PRINCIPAL
  // waveSpeed (ej. 0.03) hace que todo fluya rítmicamente hacia un lado
  vec2 p2 = p - (time * waveSpeed);
  
  // Sumar (time * 0.05) crea un Morphing cruzado (las olas chocan entre sí)
  return fbm(p + fbm(p2) + (time * 0.05)); 
}
...
interface WaveUniforms {
  ...
  bgColor: THREE.Uniform<THREE.Color>; // <-- AGREGADO
...
interface DitheredWavesProps {
  ...
  bgColor: [number, number, number]; // <-- AGREGADO
  colorNum: number;
  pixelSize: number;
...
function DitheredWaves({
  ...
  const waveUniformsRef = useRef<WaveUniforms>({
    ...
    resolution: new THREE.Uniform(new THREE.Vector2(1000, 1000)),
    waveSpeed: new THREE.Uniform(waveSpeed),
    waveFrequency: new THREE.Uniform(waveFrequency),
    waveAmplitude: new THREE.Uniform(waveAmplitude),
    waveColor: new THREE.Uniform(new THREE.Color(...waveColor)),
    bgColor: new THREE.Uniform(new THREE.Color(...bgColor)), // <-- AGREGADO
  ...
  useEffect(() => {
    ...
    if (mesh.current && mesh.current.material) {
      ...
      if (mat.uniforms.resolution.value.x !== newWidth || mat.uniforms.resolution.value.y !== newHeight) {
  ...
  const prevBgColor = useRef([...bgColor]); // <-- AGREGADO
  ...
  useFrame(({ clock }) => {
    if (mesh.current && mesh.current.material) {
      ...
      // Sincronizar el color de fondo en tiempo real
  ...
  return (
    <>
      ...
      <mesh onPointerMove={handlePointerMove} position={[0, 0, 0.01]} scale={[viewport.width, viewport.height, 1]}>
        <planeGeometry args={[1, 1]} />
...
export default function Dither({
  ...
  bgColor = [0.0, 0.0, 0.0], // Default negro
  colorNum = 4,
  pixelSize = 2,
